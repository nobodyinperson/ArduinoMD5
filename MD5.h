...
/*
 * Slightly modified by Yann Büchau <nobodyinperson@gmx.de> to avoid dynamic
 * memory allocation
 */
...
class MD5 {
 public:
  ...
  static void make_hash(unsigned char *hash, const char *arg, size_t size);
  static void make_digest(const unsigned char *digest, size_t len,
                          char *md5str);
...
void MD5::make_digest(const unsigned char *hash, size_t len, char *md5str) {
  ...
  for (size_t i = 0; i < len; i++) {
    md5str[i * 2] = hexits[hash[i] >> 4];
    md5str[(i * 2) + 1] = hexits[hash[i] & 0x0F];
  }
  md5str[len * 2] = '\0';
}
...
void MD5::make_hash(unsigned char *hash, const char *arg) {
  MD5_CTX context;
  MD5Init(&context);
  MD5Update(&context, arg, strlen(arg));
  MD5Final(hash, &context);
}
void MD5::make_hash(unsigned char *hash, const char *arg, size_t size) {
  MD5_CTX context;
  MD5Init(&context);
